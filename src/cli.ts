type OptionHandler = (data: {[s: string]: string}, getArg: () => string) => any;
class Option {
    constructor(
        public names: string[], 
        public dest: string, 
        public cb: OptionHandler
    ){}
    getName(){
...
export function arg(argv){
    ...
    return {
        ...
        opt: function(pattern, desc, cb, req){
            ...
            var nopt = new Option(parts, desc, cb/*, !!req*/);
        ...
        commonOpt: function(pattern, desc, cb, req){
            var parts = pattern.split('|');
            var nopt = new Option(parts, desc, cb);
